...
${indent}outline-offset: 2px;
    ...
    generateVariables(genome, indent, newline) {
        ...
        const parsePx = (v) => typeof v === "number" ? v : parseFloat(String(v));
        const toRem = (px) => (px / 16).toFixed(3).replace(/\.?0+$/, "");
        // clamp(min, preferred_vw, max) — preferred is ~1% per 4px of max size
        const fluidType = (size) => {
...
${indent}transform: translateY(-${hoverLift}px);
    ...
    generatePageSectionStyles(genome, indent, newline) {
        const parts = [];
        const edge = genome.chromosomes.ch7_edge;
        // ── Navigation ─────────────────────────────────────────────
...
${indent}}
    ...
    generateChoreographyStyles(genome, indent, newline) {
...
${indent}--easing-choreography: ${easing};
        ...
        const staggerLines = Array.from({ length: 8 }, (_, i) => `.stagger-child:nth-child(${i + 1}) { transition-delay: ${staggerMs * i}ms; animation-delay: ${staggerMs * i}ms; }`).join('\n');
    ...
    generateIconographyStyles(genome, indent, newline) {
        const icon = genome.chromosomes.ch28_iconography;
        const parts = [];
        parts.push(`/* Iconography - ch28 */`);
        const strokeWeightMap = { thin: 1, regular: 1.5, bold: 2.5, variable: 2 };
        const strokePx = strokeWeightMap[icon.strokeWeight] ?? 1.5;
        const colorTreatmentMap = {
        ...
        const cornerMap = { sharp: '0', rounded: 'var(--radius-sm)', pill: 'var(--radius-full)' };
...
${indent}flex-shrink: 0;
    ...
    generateTextureStyles(genome, indent, newline) {
        const texture = genome.chromosomes.ch11_texture;
        const parts = [];
        if (texture.surface === 'flat')
            return '';
