...
export class SVGGenerator {
    async generateBiomarker(genome, callText) {
...
The mark should feel: ${shapeChar}. It should look like it belongs to a ${genome.sectorContext.primary} product with ${edge.style} edge treatment. The ${bm.animationStyle} animation soul should be visible in its static form as visual tension or implied movement.`;
    ...
    extractAndSanitize(raw) {
        ...
        content = content.replace(/<(\w+)([^>]*)\/?>[\s\S]*?<\/\1>|<(\w+)([^>]*)\/>/g, (match, tag1, _attrs1, tag2) => {
  ...
  <g stroke-width="${weight.toFixed(1)}" fill="none" stroke="currentColor">
    ...
    fallbackGeometric(complexity, entropy, bytes) {
    ...
    fallbackCrystalline(complexity, entropy, bytes) {
    ...
    fallbackBiological(complexity, entropy, bytes) {
    ...
    fallbackFluid(complexity, entropy, bytes) {
        ...
        for (let i = 0; i < lines; i++) {
            ...
            paths.push(`<path d="M 10,${yOffset.toFixed(1)} C ${(10 + amplitude + phase).toFixed(1)},${(yOffset - amplitude).toFixed(1)} ${(90 - amplitude - phase).toFixed(1)},${(yOffset + amplitude).toFixed(1)} 90,${yOffset.toFixed(1)}" stroke-linecap="round" />`);
        }
        return paths.join("\n    ");
    }
    // Architectural: asymmetric composed rectangles — structure, weight
    fallbackArchitectural(complexity, entropy, bytes) {
