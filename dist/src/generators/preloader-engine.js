...
export function generatePreloaderOutput(genome) {
...
// ── HTML ──────────────────────────────────────────────────────────────────────
function generatePreloaderHTML(type, companyName) {
    switch (type) {
        case "minimal_bar":
        ...
        case "word_scatter":
            return `<div class="preloader preloader--words" aria-hidden="true">${["DESIGN", "SYSTEM", "GENOME", companyName || "BRAND"].map((w, i) => `<span class="preloader-word" style="--word-i:${i}">${w}</span>`).join("")}</div>`;
        case "svg_draw":
        ...
        case "grid_fill":
            return `<div class="preloader preloader--grid" aria-hidden="true">${Array.from({ length: 25 }, (_, i) => `<div class="preloader-cell" style="--cell-i:${i}"></div>`).join("")}</div>`;
        case "3d_scene_spin":
...
// ── CSS ───────────────────────────────────────────────────────────────────────
function generatePreloaderCSS(type) {
    const base = `
/* ── Preloader ────────────────────────────────────────────────── */
.preloader {
...
function generatePreloaderTypeCSS(type) {
    switch (type) {
        case "minimal_bar":
            return `
...
// ── JS ────────────────────────────────────────────────────────────────────────
function generatePreloaderJS(type, companyName) {
    const dismiss = `
