...
export type ScrollTechnique =
    ...
    | "parallax_layers"
    | "velocity_distort"
    | "native";  // plain browser scroll — no JS

// ── Catalog entry ─────────────────────────────────────────────────────────────

export interface ScrollEntry {
    technique: ScrollTechnique;
    description: string;
    /** npm packages required (empty = no install needed) */
    requires: string[];
    forbiddenFor: {
        philosophies?: DesignPhilosophy[];
        requiresPhysics?: string[];     // must be one of these
        requiresPhysicsNot?: string[];  // must NOT be one of these
        requiresScrollBehavior?: string[]; // ch1.scrollBehavior must be one of these
        requiresWebGL?: boolean;
        minSectionCount?: number;
    };
}

export const SCROLL_CATALOG: ScrollEntry[] = [
    {
        technique: "native",
...
export function getScrollEntry(technique: ScrollTechnique): ScrollEntry {
...
export function selectScrollTechnique(opts: {
    ...
    if (physics === "none") {
        if (scrollBehavior === "snap" || scrollBehavior === "paginated") return "scroll_snap_v";
        return "css_scroll_driven";
    }

    // minimalist → native
    if (philosophy === "minimalist") return "native";

    // snap / paginated
    if (scrollBehavior === "snap" || scrollBehavior === "paginated") return "scroll_snap_v";

    // WebGL + velocity
    if (hasWebGL && physics !== "none") return "velocity_distort";
    ...
    // continuous
    if (scrollBehavior === "continuous") return "lenis_gsap_scrub";
