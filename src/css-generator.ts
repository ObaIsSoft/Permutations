 ...
 * 
 * NOW WITH: Persona influence - the simulated designer's aesthetic
 ...
 * Mirrors PersonaInfluence from bridge/persona-to-design.ts
 */
export interface PersonaCSSInfluence {
    hueShift?: number;           // ±30° shift to primary hue
    saturationMod?: number;      // ±0.3 saturation adjustment
    lightnessMod?: number;       // ±0.2 lightness adjustment
    densityMod?: number;         // ±0.4 spacing/density adjustment
    whitespaceMod?: number;      // ±0.3 whitespace adjustment
    radiusMod?: number;          // ±8px border radius adjustment
    elevationMod?: number;       // ±0.5 shadow elevation adjustment
    trackingMod?: number;        // ±0.02 letter-spacing adjustment
    physicsStyle?: 'spring' | 'glitch' | 'step' | 'none';  // Motion personality
    copyTone?: string;           // Copy voice (for CSS comments/content)
    metaphorPrimary?: string;    // Design metaphor (for CSS custom properties)
}
...
export interface CSSGenerationOptions {
    ...
    personaInfluence?: PersonaCSSInfluence;  // L0 persona aesthetic influence
}
...
export class CSSGenerator {
    ...
    generate(genome: DesignGenome, options: CSSGenerationOptions = {}): string {
        ...
        const { includeReset = true, includeVariables = true, format = "expanded", personaInfluence } = options;
        const indent = format === "expanded" ? "  " : "";
        const newline = format === "expanded" ? "\n" : "";
        
        const parts: string[] = [];
        
        // Add persona attribution comment if present
        if (personaInfluence?.copyTone) {
            parts.push(`/* Generated by persona with ${personaInfluence.copyTone} voice */`);
            parts.push(`/* Metaphor: ${personaInfluence.metaphorPrimary || 'none'} */`);
        }
        ...
        if (includeVariables) {
            parts.push(this.generateVariables(genome, indent, newline, personaInfluence));
        }
...
${indent}outline-offset: 2px;
    ...
    private generateVariables(genome: DesignGenome, indent: string, newline: string, influence?: PersonaCSSInfluence): string {
        const { chromosomes } = genome;
        const parts: string[] = [];
        
        parts.push(`:root {`);
        
        // Apply persona influence to colors
        let primaryHue = Math.round(chromosomes.ch5_color_primary.hue);
        let primarySat = Math.round(chromosomes.ch5_color_primary.saturation * 100);
        let primaryLight = Math.round(chromosomes.ch5_color_primary.lightness * 100);
        
        // Persona hue shift (±30°)
        if (influence?.hueShift) {
            primaryHue = (primaryHue + influence.hueShift + 360) % 360;
        }
        // Persona saturation modification (±30%)
        if (influence?.saturationMod) {
            primarySat = Math.max(0, Math.min(100, primarySat + (influence.saturationMod * 100)));
        }
        // Persona lightness modification (±20%)
        if (influence?.lightnessMod) {
            primaryLight = Math.max(5, Math.min(95, primaryLight + (influence.lightnessMod * 100)));
        }
        ...
        // Spacing - apply persona density/whitespace influence
        const rhythm = chromosomes.ch2_rhythm;
        let baseSpacing = rhythm.baseSpacing;
        // Persona density modification (±40%)
        if (influence?.densityMod) {
            baseSpacing = baseSpacing * (1 + influence.densityMod);
        }
        // Persona whitespace modification (±30%)
        ...
        if (influence?.radiusMod) {
            edgeRadius = Math.max(0, edgeRadius + influence.radiusMod);
        }
        parts.push(`${indent}/* Radii */`);
        parts.push(`${indent}--radius-sm: ${Math.max(0, Math.round(edgeRadius * 0.5))}px;`);
        parts.push(`${indent}--radius-md: ${Math.round(edgeRadius)}px;`);
        parts.push(`${indent}--radius-lg: ${Math.round(edgeRadius * 2)}px;`);
        parts.push(`${indent}--radius-xl: ${Math.round(edgeRadius * 4)}px;`);
        parts.push(`${indent}--radius-full: 9999px;`);
        
        // Motion - apply persona physics style
        const motion = chromosomes.ch8_motion;
        let durationScale = motion.durationScale;
        // Persona physics affects timing
        if (influence?.physicsStyle === 'glitch') {
            durationScale *= 0.5; // Faster, more chaotic
        } else if (influence?.physicsStyle === 'step') {
            durationScale *= 1.5; // Slower, more deliberate
        }
        parts.push(`${indent}/* Motion */`);
        parts.push(`${indent}--duration-fast: ${(0.15 * durationScale).toFixed(2)}s;`);
        parts.push(`${indent}--duration-normal: ${(0.3 * durationScale).toFixed(2)}s;`);
        parts.push(`${indent}--duration-slow: ${(0.6 * durationScale).toFixed(2)}s;`);
        // Persona physics style affects easing
        if (influence?.physicsStyle === 'glitch') {
            parts.push(`${indent}--ease-spring: steps(4, end); /* glitch persona */`);
        } else if (influence?.physicsStyle === 'step') {
            parts.push(`${indent}--ease-spring: cubic-bezier(0.8, 0, 0.2, 1); /* step persona */`);
        } else {
