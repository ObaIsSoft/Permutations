...
export interface PostProcessingEntry {
    ...
    description: string;
    /** @react-three/postprocessing package name */
    effectName: string;
    activatedBy: {
        material?: string[];
        physics?: string[];
...
export const POST_PROCESSING_CATALOG: PostProcessingEntry[] = [
    {
        pass: "bloom_subtle",
...
export function selectPostProcessingPasses(opts: {
    ...
    const result: PostProcessingPass[] = [];
    ...
    for (const entry of POST_PROCESSING_CATALOG) {
        ...
        if (f.maxEntropy !== undefined && entropy > f.maxEntropy) continue;

        // Activation check — any match is sufficient
        let activated = false;
        if (a.material?.includes(material)) activated = true;
        if (a.physics?.includes(physics)) activated = true;
        if (a.fx?.includes(fx)) activated = true;
        if (a.minNoiseLevel !== undefined && noiseLevel >= a.minNoiseLevel) activated = true;
        if (a.minShadowScale !== undefined && shadowScale >= a.minShadowScale) activated = true;
        if (a.minEntropy !== undefined && entropy >= a.minEntropy) activated = true;

        if (activated) result.push(entry.pass);
    }
