...
export class PatternDetector {
    ...
    detectInGenome(genome: any, css: string, html?: string): PatternViolation[] {
        const violations = this.detect(css, html);
        const enforced: PatternViolation[] = [];

        if (genome?.constraints?.forbiddenPatterns) {
            genome.constraints.forbiddenPatterns.forEach((pattern: string) => {
