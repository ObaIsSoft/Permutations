...
import type { DesignPersonality, EdgeStyle, MotionPhysics } from "./genome/types.js";
import type {
    SuccessionStage, SymbiosisPattern, TrophicStructure,
    PopulationPattern, AdaptationAxis, BiomeClass,
} from "./genome/ecosystem-types.js";

// ── Taxonomy ─────────────────────────────────────────────────────────────────

export type OrganismTier =
    | "microbial"   // atomic: buttons, badges, avatars, chips, progress
    | "flora"       // composite: inputs, cards, menus, dropdowns, forms
    | "fauna"       // complex: modals, tables, date pickers, comboboxes
    | "positioning" // utility: tooltip / popover / dropdown placement
    | "any";        // covers all tiers

export type OrganismPhilosophy =
    | "headless"    // unstyled — full control, zero default styles
    | "styled"      // ships with opinions — override via tokens
    | "system"      // full design system — opinionated layout + tokens
    | "positioning" // positional utility only (Floating UI)
    | "primitive";  // no library — raw HTML + ARIA attributes

// ── Entry ─────────────────────────────────────────────────────────────────────

export interface OrganismLibraryEntry {
    name: string;
    package: string;
    version: string;
    bundleSize: string;           // runtime cost (0kb = build-time / CSS only)
    license: "MIT" | "Apache-2.0" | "ISC";
    philosophy: OrganismPhilosophy;
    description: string;          // one-line character
    devxScore: number;            // 0–1, developer ergonomics baseline
    tiers: OrganismTier[];        // which organism tiers it covers
    ssrSafe: boolean;
    typescript: "first-class" | "supported" | "partial" | "not-needed";
    installCmd: string;
    importExample: string;
    minimalExample: string;
    combinableWith?: string[];    // package names that pair well with this lib
    /**
     * What this library is WRONG for — hard exclusion gate (pre-filter).
    ...
    /**
     * Scoring boost fields — ALL additive, applied within the eligible pool.
     * Any of these can be combined; missing them just means no bonus.
     */
...
export const ORGANISM_LIBRARY_CATALOG: OrganismLibraryEntry[] = [
...
export interface OrganismGenomeInput {
    // L1 design chromosomes
    edgeStyle: EdgeStyle;
    motionPhysics: MotionPhysics;
    // L2 ecosystem chromosomes
...
export interface OrganismSelection {
    primary: OrganismLibraryEntry;
    alternative: OrganismLibraryEntry;
    also_consider: OrganismLibraryEntry[];
}
 ...
 */
export function selectOrganismLibrary(input: OrganismGenomeInput): OrganismSelection {
    ...
    const eligible = ORGANISM_LIBRARY_CATALOG.filter(lib => {
        ...
        if (f.personalities?.includes(input.personality)) return false;
    ...
    function score(lib: OrganismLibraryEntry): number {
        let s = lib.devxScore * 50; // 0–50 base from devx quality

        // L1 bonuses (design genome)
        if (lib.fitsEdge?.includes(input.edgeStyle))        s += 15;
        if (lib.fitsMotion?.includes(input.motionPhysics))  s += 10;
        ...
        if (lib.fitsSymbiosis?.includes(input.symbiosis))       s += 10;
        if (lib.fitsTrophic?.includes(input.trophic))           s += 10;
        if (lib.fitsPopulation?.includes(input.population))     s += 10;
        if (lib.fitsBiome?.includes(input.biome))               s += 10;
        if (lib.fitsAdaptation?.includes(input.adaptation))     s += 10;

        // Tier relevance
        if (input.hasFauna && (lib.tiers.includes("fauna") || lib.tiers.includes("any"))) s += 5;
