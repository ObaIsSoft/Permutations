...
import type { StateTopology } from "./genome/types.js";

// ── State paradigm taxonomy ───────────────────────────────────────────────────

export type StateParadigm =
    | "local"        // useState / useReducer — component-scoped
    | "context"      // React Context — cross-component, no external dep
    | "flux"         // Unidirectional store — Redux, Zustand
    | "reactive"     // Signal-based / observable — Jotai, Valtio, MobX
    | "server"       // Server state / cache — TanStack Query, SWR
    | "machine"      // State machine — XState, Robot
    | "federated";   // Cross-app / micro-frontend — Module Federation

// ── Library entry ─────────────────────────────────────────────────────────────

export interface StateLibraryEntry {
    name: string;
    package: string;
    version: string;
    bundleSize: string;      // approx gzipped
    license: "MIT" | "Apache-2.0";
    paradigm: StateParadigm;
    description: string;     // one-line design character
    devxScore: number;       // 0–1, developer ergonomics (quality-weighted tiebreaker)
    installCmd: string;
    importExample: string;
    minimalExample: string;
    reactFirst: boolean;     // requires React or works anywhere
     ...
     */
    forbiddenFor: {
        /** WRONG for these state topologies — architectural mismatch */
        topologies?: StateTopology[];
...
export const STATE_LIBRARY_CATALOG: StateLibraryEntry[] = [
    // ── Local / no-library ──────────────────────────────────────────────────
    {
 ...
 */
export function selectStateLibrary(
    topology: StateTopology,
    complexityScore: number,
    dnaHashByte: number
): { primary: StateLibraryEntry; alternative: StateLibraryEntry } {
    const eligible = STATE_LIBRARY_CATALOG.filter(lib => {
        const f = lib.forbiddenFor;
        if (f.topologies?.includes(topology)) return false;
