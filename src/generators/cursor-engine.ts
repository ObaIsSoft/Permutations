...
import type { CursorType } from "../cursor-catalog.js";

// ── Public API ────────────────────────────────────────────────────────────────

export interface CursorEngineOutput {
    /** CSS to inject into stylesheet — @media (pointer: fine) wrapped */
    css: string;
    /** JS to inject into app.js */
    js: string;
    /** HTML element to inject at end of <body> */
    html: string;
    /** The resolved cursor type */
    cursorType: CursorType;
}

export function generateCursorOutput(genome: DesignGenome): CursorEngineOutput {
...
function generateCursorCSS(type: CursorType): string {
    const base = `
/* ── Custom Cursor ──────────────────────────────────────────── */
...
function generateCursorTypeCSS(type: CursorType): string {
    switch (type) {
        case "dot_circle":
            return `
  .cursor-dot {
...
function generateCursorHTML(type: CursorType): string {
    switch (type) {
        case "dot_circle":
        ...
        case "trail_echo":
            return Array.from({ length: 7 }, (_, i) =>
                `  <div class="cursor-trail-dot" data-trail="${i}" aria-hidden="true"></div>`
            ).join("\n");
        case "magnetic_pull":
            return `  <div class="cursor-dot" aria-hidden="true"></div>`;
        default:
...
function generateCursorJS(type: CursorType, magneticRange: number): string {
    switch (type) {
        case "dot_circle":
            return `
// cursor: dot_circle
