...
import type { ScrollTechnique } from "../scroll-catalog.js";

// ── Public API ────────────────────────────────────────────────────────────────

export interface ScrollEngineOutput {
    css: string;
    js: string;
    /** npm install line, empty if no deps */
    installHint: string;
    technique: ScrollTechnique;
}

export function generateScrollOutput(genome: DesignGenome): ScrollEngineOutput {
    const ch1 = genome.chromosomes.ch1_structure as any;
...
function generateScrollCSS(technique: ScrollTechnique): string {
    switch (technique) {
        case "scroll_snap_v":
            return `
...
function generateScrollJS(technique: ScrollTechnique): string {
    switch (technique) {
        case "native":
...
function getInstallHint(technique: ScrollTechnique): string {
    const deps: Record<ScrollTechnique, string> = {
