...
import type { PageCompositionSpec, SectionSpec, ComponentSpec, LibrarySelection } from "../genome/context-composer.js";
...
export interface ComponentFile { path: string; content: string; }
export interface StyleFile { path: string; content: string; }
...
export class ReactGenerator {
    ...
    private generatePage(spec: PageCompositionSpec): PageFile {
...
`,
    ...
    private renderBlueprint(blueprint: { template: string; styles: string; dependencies: string[] }, v: ReturnType<typeof gv>, animConfig: ReturnType<typeof getAnimationConfig>, indent: string, delay: number): string {
        const props: Record<string, string> = {
            ...
            stats: (v.stats || []).map((s: any) => `<div className="stat"><span className="stat-value">${s.value}</span><span className="stat-label">${s.label}</span></div>`).join("\n"),
            featureCards: (v.features || []).map((f: any) => `<div className="feature-card"><div className="feature-icon"></div><h3 className="feature-title">${f.title}</h3><p className="feature-description">${f.description}</p></div>`).join("\n"),
            faqItems: (v.faq || []).map((f: any) => `<details className="faq-item"><summary className="faq-question">${f.question}</summary><div className="faq-answer"><p>${f.answer}</p></div></details>`).join("\n"),
            testimonialCards: "",
            pricingCards: "",
            teamCards: "",
            blogCards: "",
    ...
    private renderNavFallback(nav: any, v: ReturnType<typeof gv>, animConfig: ReturnType<typeof getAnimationConfig>, indent: string, delay: number): string {
        const items = (nav.items || []).map((item: any) =>
            `${indent}    <a href="${item.href}" className="nav-link">${item.label}</a>`
        ).join("\n");
    ...
    private renderSectionFallback(section: SectionSpec, v: ReturnType<typeof gv>, animConfig: ReturnType<typeof getAnimationConfig>, indent: string, delay: number): string {
        ...
        if (component) {
            const code = generateComponentCode(component, v as any, component.variants[0]?.name || "default");
    ...
    private findComponentForSection(sectionType: string): any {
    ...
    private generateComponents(spec: PageCompositionSpec): ComponentFile[] {
        const v = gv(spec.genome);
...
export interface ${component.name}Props {
  ${component.props.filter((p: any) => !p.genomeAdaptive).map((p: any) => `${p.name}${p.required ? "" : "?"}: ${p.type};`).join("\n  ")}
}

export function ${component.name}({ ${component.props.filter((p: any) => !p.genomeAdaptive).map((p: any) => p.name).join(", ")} }: ${component.name}Props) {
  return (
    ${code.react}
  );
}
`,
    ...
    private generateStyles(spec: PageCompositionSpec): StyleFile {
        const v = gv(spec.genome);
        const animConfig = getAnimationConfig(spec.genome);
...
`,
    ...
    private generateConfig(spec: PageCompositionSpec): ConfigFile {
