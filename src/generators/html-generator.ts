...
${bodyContent}
  <script src="app.js" defer></script>
    ...
    private renderBody(v: ReturnType<typeof gv>, animConfig: ReturnType<typeof getAnimationConfig>): string {
        ...
        const indent = "  ";
        let html = "";
        if (layout.pattern?.blueprint) html += this.renderBlueprint(layout.pattern.blueprint, v, animConfig, indent, 0);
        else html += `${indent}<div class="page-layout layout-${layout.type}">\n`;
        if (navigation?.pattern?.blueprint) html += this.renderBlueprint(navigation.pattern.blueprint, v, animConfig, indent + "  ", 0);
        else if (navigation) html += this.renderNavFallback(navigation, v, animConfig, indent + "  ", 0);
        html += `${indent}  <main class="main-content">\n`;
        if (sidebar?.pattern?.blueprint) html += this.renderBlueprint(sidebar.pattern.blueprint, v, animConfig, indent + "    ", 0);
        if (hero?.pattern?.blueprint) html += this.renderBlueprint(hero.pattern.blueprint, v, animConfig, indent + "    ", 0);
        for (let i = 0; i < sections.length; i++) {
            const section = sections[i];
            const delay = i * animConfig.staggerInterval;
            if (section.pattern?.blueprint) html += this.renderBlueprint(section.pattern.blueprint, v, animConfig, indent + "    ", delay);
            else html += this.renderSectionFallback(section, v, animConfig, indent + "    ", delay);
        }
        html += `${indent}  </main>\n`;
        if (footer?.pattern?.blueprint) html += this.renderBlueprint(footer.pattern.blueprint, v, animConfig, indent + "  ", sections.length * animConfig.staggerInterval);
        html += `${indent}</div>`;
        return html;
    }
    ...
    private generateCSS(v: ReturnType<typeof gv>, animConfig: ReturnType<typeof getAnimationConfig>): string {
        ...
        for (const pattern of allPatterns) if (pattern?.blueprint?.styles) patternStyles.add(pattern.blueprint.styles);
