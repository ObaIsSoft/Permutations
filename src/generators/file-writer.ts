...
export interface FileOutput {
    path: string;
    content: string;
    type: "component" | "style" | "config" | "asset" | "preview";
}
...
export class DesignFileWriter {
    ...
    async writeDesignSystem(
        ...
        try {
            ...
            if (cssContent) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "styles", `genome.${options.styling === "scss" ? "scss" : "css"}`),
                    cssContent,
                    result
                ));
            }
            ...
            if (outputs.components) {
                ...
                writes.push(this.writeFile(
                    path.join(options.baseDir, "components", `DesignSystem.${ext}`),
                    outputs.components,
                    result
                ));
            }
            ...
            if (outputs.animations) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "lib", "animations.ts"),
                    outputs.animations,
                    result
                ));
            }
            ...
            if (outputs.architecture) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "lib", "architecture.ts"),
            ...
            if (outputs.interactions) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "lib", "interactions.ts"),
            ...
            if (outputs.index) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "index.ts"),
                    outputs.index,
                    result
                ));
            }
            ...
            if (outputs.html) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "preview", "index.html"),
                    outputs.html,
                    result
                ));
            }
            ...
            if (outputs.webgl) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "components", "Procedural3D.tsx"),
                    outputs.webgl,
                    result
                ));
            }
            ...
            if (outputs.fx) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "styles", "atmosphere.css"),
                    outputs.fx,
                    result
                ));
            }
            ...
            if (outputs.svg) {
                writes.push(this.writeFile(
                    path.join(options.baseDir, "assets", "biomarker.svg"),
                    outputs.svg,
                    result
                ));
            }
            ...
            writes.push(this.writeFile(
                path.join(options.baseDir, "genome.json"),
    ...
    async writeComponentFiles(
        ...
        genome: DesignGenome,
        baseDir: string,
        options: { typescript?: boolean; framework?: "react" | "vue" | "svelte" } = {}
    ): Promise<WrittenFiles> {
        const result: WrittenFiles = {
            success: true,
            files: [],
        ...
        try {
            ...
            await this.writeFile(
                path.join(componentsDir, `index.${ext}`),
                exports,
                result
            );
     ...
     */
    generatePreviewHTML(genome: DesignGenome, css: string, html: string): string {
...
<body>
     ...
     */
    generateFileStructure(files: string[], baseDir: string): string {
    ...
    private async writeFile(filePath: string, content: string, result: WrittenFiles): Promise<void> {
        try {
    ...
    private generateReactComponent(spec: ComponentSpec, genome: DesignGenome, typescript?: boolean): string {
...
`;
    ...
    private generateVueComponent(spec: ComponentSpec, genome: DesignGenome, typescript?: boolean): string {
        const edgeRadius = genome.chromosomes.ch7_edge.radius;
        const primary = genome.chromosomes.ch5_color_primary;
...
`;
    ...
    private generateSvelteComponent(spec: ComponentSpec, genome: DesignGenome): string {
        const edgeRadius = genome.chromosomes.ch7_edge.radius;
        const primary = genome.chromosomes.ch5_color_primary;
