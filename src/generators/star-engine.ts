...
import type { StarType } from "../genome/types.js";
import { getStarEntry } from "../star-catalog.js";

// ── Public API ────────────────────────────────────────────────────────────────

export interface StarHeroHTML {
    /** The complete hero element HTML string */
    html: string;
    /** data attributes to stamp on the <section class="hero"> element */
 ...
 */
export function renderStarHero(
    genome: DesignGenome,
    v: {
        headline?: string;
        subheadline?: string;
    ...
    const starType = (genome.chromosomes.ch19_hero_type?.starType ?? "none") as StarType;
 ...
 */
function renderStarElement(starType: StarType, v: {
    headline?: string;
    companyName?: string;
