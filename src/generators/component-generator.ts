...
export interface ComponentDescription {
    purpose: string;           // "pricing", "navigation", "content_display"
    elements: string[];        // ["title", "price", "cta", "feature_list"]
    layout: "horizontal" | "vertical" | "grid" | "layered";
...
export class ComponentGenerator {
    ...
    generate(
        ...
        const b = (idx: number) => this.getHashByte(componentSeed + description.purpose, idx);
    ...
    private inferStructure(
        ...
        const nestingDepth = Math.min(
            desc.elements.length,
            Math.floor(b(2) * c.ch1_structure.maxNesting) + 1
        );
    ...
    private generateHTML(
        desc: ComponentDescription,
        structure: { container: string; elementOrder: string[]; nestingDepth: number },
        c: DesignGenome["chromosomes"],
        b: (i: number) => number
    ): string {
  ...
  <div class="dna-${desc.purpose}__container">
    ...
    private generateCSS(
        desc: ComponentDescription,
        structure: { container: string; elementOrder: string[]; nestingDepth: number; gridConfig?: { columns: number; gap: number } },
        c: DesignGenome["chromosomes"],
        b: (i: number) => number
    ): string {
...
.dna-${elementType}:focus {
  ...
  box-shadow: 0 0 0 ${focusRing}px var(--primary)${Math.floor(20 + b(8) * 40).toString(16).padStart(2, '0')};
    ...
    private generateMotionStyles(
        componentName: string,
        motion: NonNullable<DesignGenome["chromosomes"]["ch27_motion_choreography"]>,
        physics: string,
        b: (i: number) => number
    ): string {
...
.dna-${componentName} [class*="cta"]:hover {
  transition: transform ${motion.hoverMicrointeraction.duration}ms ${physics};
    ...
    private layoutToDisplay(layout: string): string {
        switch (layout) {
    ...
    private generateLayoutStyles(
        layout: string,
        gridConfig: { columns: number; gap: number } | undefined,
        c: DesignGenome["chromosomes"],
        b: (i: number) => number
    ): string {
        switch (layout) {
    ...
    private generateAttributes(desc: ComponentDescription, c: DesignGenome["chromosomes"]): string {
        const attrs: string[] = [];
    ...
    private inferAccessibility(desc: ComponentDescription): { roles: string[]; ariaLabels: Record<string, string> } {
        const roles: string[] = [];
        const ariaLabels: Record<string, string> = {};

        if (desc.purpose === "navigation") roles.push("navigation");
        if (desc.purpose === "pricing") roles.push("region");
        if (desc.elements.includes("testimonial")) roles.push("blockquote");
        if (desc.layout === "grid") roles.push("grid");

        return { roles, ariaLabels };
    }

    private generateName(desc: ComponentDescription, c: DesignGenome["chromosomes"], b: (i: number) => number): string {
