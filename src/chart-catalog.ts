...
import type {
    AdaptationAxis, TrophicStructure, SuccessionStage,
    EnergySource, BiomeClass,
} from "./genome/ecosystem-types.js";

// ── Taxonomy ─────────────────────────────────────────────────────────────────

export type ChartFamily =
    | "statistical"    // bar, histogram, box, scatter — distribution + comparison
    | "temporal"       // line, area, stream — change over time
    | "relational"     // network, force, sankey, chord — relationships
    | "geospatial"     // map, choropleth, point cloud — location data
    | "compositional"  // pie, treemap, sunburst, waffle — part-to-whole
    | "grammar";       // low-level grammar of graphics — fully custom marks

export type ChartApproach =
    | "declarative"    // component API — <BarChart data={...}> — fastest DX
    | "grammar"        // mark-based — Plot.barY(data, {x: 'name', y: 'value'})
    | "composable"     // composable primitives — mix scales, axes, marks freely
    | "imperative";    // D3 — full control, steepest learning curve
...
export const CHART_LIBRARY_CATALOG: ChartLibraryEntry[] = [
...
export interface ChartGenomeInput {
    // L1
    edgeStyle: EdgeStyle;
    motionPhysics: MotionPhysics;
    complexityScore: number;    // 0.0–1.0 from ch15_biomarker
    // L2
    ...
    hasFauna: boolean;          // charts are only relevant when fauna tier exists
}

export interface ChartSelection {
    primary: ChartLibraryEntry;
    alternative: ChartLibraryEntry;
    also_consider: ChartLibraryEntry[];
    /** Whether fauna complexity warrants any charting at all */
    chartsRecommended: boolean;
}
...
export function selectChartLibrary(input: ChartGenomeInput): ChartSelection {
    ...
    function score(lib: ChartLibraryEntry): number {
        ...
        // L1 bonuses
        if (lib.fitsEdge?.includes(input.edgeStyle))       s += 12;
        ...
        if (lib.fitsAdaptation?.includes(input.adaptation))   s += 15;
