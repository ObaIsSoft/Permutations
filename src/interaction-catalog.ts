...
import type { DesignPersonality, MotionPhysics } from "./genome/types.js";
import type {
    TrophicStructure, PopulationPattern, SuccessionStage,
    AdaptationAxis, EnergySource, SymbiosisPattern,
} from "./genome/ecosystem-types.js";

// ── Taxonomy ─────────────────────────────────────────────────────────────────

export type InteractionDomain =
    | "drag_drop"        // reorderable lists, sortable grids, kanban, tree DnD
    | "gesture"          // multi-touch, swipe, pinch-zoom, rotate, scroll binding
    | "virtual_scroll"   // windowed lists/grids — render only visible rows
    | "keyboard"         // hotkeys, shortcuts, chord sequences
    | "focus_management" // focus trapping, roving tabindex, focus restoration
    | "pointer";         // pointer events, cursor tracking, magnetic effects

// ── Entry ─────────────────────────────────────────────────────────────────────

export interface InteractionLibraryEntry {
    name: string;
    package: string;
    version: string;
    bundleSize: string;
    license: "MIT" | "Apache-2.0" | "ISC";
    domain: InteractionDomain[];      // one library may cover multiple domains
    description: string;
    devxScore: number;
    ssrSafe: boolean;                 // safe to import in SSR context
    typescript: "first-class" | "supported" | "partial";
    installCmd: string;
    importExample: string;
    minimalExample: string;
    combinableWith?: string[];        // packages that pair naturally with this
    /**
    ...
    fitsMotion?: MotionPhysics[];
    fitsSuccession?: SuccessionStage[];
    fitsTrophic?: TrophicStructure[];
    fitsPopulation?: PopulationPattern[];
    fitsAdaptation?: AdaptationAxis[];
    fitsEnergy?: EnergySource[];
    fitsSymbiosis?: SymbiosisPattern[];
}

// ── Catalog ───────────────────────────────────────────────────────────────────

export const INTERACTION_LIBRARY_CATALOG: InteractionLibraryEntry[] = [
...
export interface InteractionGenomeInput {
    // L1
    motionPhysics: MotionPhysics;
    // L2
    trophic: TrophicStructure;
    population: PopulationPattern;
    succession: SuccessionStage;
    adaptation: AdaptationAxis;
    energy: EnergySource;
    symbiosis: SymbiosisPattern;
    personality: DesignPersonality;
    // Context
    hasFauna: boolean;           // fauna-tier components imply virtual scroll / DnD need
    hasDataHeavyFauna: boolean;  // tables, kanban → DnD + virtual scroll strongly needed
    /** Used as hard-exclusion gate in forbiddenFor — default 0.5 if not provided */
    complexity?: number;
}

export interface InteractionSelection {
    /** Primary recommendation — highest chromosome alignment */
    primary: InteractionLibraryEntry;
    /** Secondary recommendation — different domain from primary */
    alternative: InteractionLibraryEntry;
    /** Full ranked list — all libraries scored, order from highest to lowest */
    ranked: InteractionLibraryEntry[];
    /** Domain coverage map — which domains are covered by primary + alternative */
    coverage: InteractionDomain[];
}
 ...
 */
export function selectInteractionLibraries(input: InteractionGenomeInput): InteractionSelection {
    const complexity = input.complexity ?? 0.5;

    // Hard exclusion gate
    const eligible = INTERACTION_LIBRARY_CATALOG.filter(lib => {
        const f = lib.forbiddenFor;
        if (f.physics?.includes(input.motionPhysics)) return false;
    ...
    function score(lib: InteractionLibraryEntry): number {
        let s = lib.devxScore * 50;

        // L1
        if (lib.fitsMotion?.includes(input.motionPhysics)) s += 15;

        // L2
        if (lib.fitsPersonality?.includes(input.personality))    s += 20;
        if (lib.fitsSuccession?.includes(input.succession))      s += 15;
        if (lib.fitsTrophic?.includes(input.trophic))            s += 12;
        if (lib.fitsPopulation?.includes(input.population))      s += 10;
        if (lib.fitsAdaptation?.includes(input.adaptation))      s += 10;
        if (lib.fitsEnergy?.includes(input.energy))              s += 8;
        if (lib.fitsSymbiosis?.includes(input.symbiosis))        s += 8;

        // Context boosts
        if (input.hasFauna && lib.domain.includes("virtual_scroll")) s += 10;
        if (input.hasDataHeavyFauna && lib.domain.includes("drag_drop")) s += 15;
        if (input.hasDataHeavyFauna && lib.domain.includes("virtual_scroll")) s += 15;
    ...
    const alternative = ranked.find(
        lib => !lib.domain.some(d => primary.domain.includes(d))
    ) ?? ranked[1];

    const coverageSet = new Set<InteractionDomain>([
        ...primary.domain,
        ...alternative.domain,
    ]);
