...
export interface TypographyTreatmentEntry {
    treatment: TypographyTreatment;
    description: string;
    /** CSS class applied to elements using this treatment */
    cssClass: string;
    /**
     * Conditions under which this treatment is NEVER used.
     * All exclusion — no whitelists.
     */
    forbiddenFor: {
        philosophies?: DesignPhilosophy[];
        /** physics modes that must NOT be active */
        requiresPhysicsNot?: string[];
        /** physics modes where this IS valid (empty = all are fine) */
        requiresPhysics?: string[];
        /** minimum entropy to activate */
        minEntropy?: number;
        /** only valid when font is variable */
        requiresVariableFont?: boolean;
    };
    /**
     * Which HTML elements this treatment applies to.
     * Used by the engine to know where to inject the class.
     */
    appliesTo: Array<"display" | "h1" | "h2" | "h3" | "body" | "stat" | "label">;
}

export const TYPOGRAPHY_TREATMENT_CATALOG: TypographyTreatmentEntry[] = [
    {
...
export function getTreatmentEntry(treatment: TypographyTreatment): TypographyTreatmentEntry | undefined {
...
export function selectTypographyTreatments(opts: {
    ...
    for (const entry of TYPOGRAPHY_TREATMENT_CATALOG) {
        ...
        if (f.requiresVariableFont && !isVariableFont) continue;
