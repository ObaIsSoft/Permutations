 ...
 */
export function validateGenome(genome: any, context: string): asserts genome is { 
    chromosomes: any; 
    sectorContext: any; 
    dnaHash: string 
} {
 ...
 */
export function validateEcosystemOutput(eco: any, context: string): void {
    if (!eco) {
        throw new McpError(ErrorCode.InvalidParams, `${context}: Missing ecosystem output`);
    }
    if (!eco.environment?.genome) {
        throw new McpError(ErrorCode.InvalidParams, 
            `${context}: Ecosystem missing environment.genome. Pass the complete ecosystem output from generate_ecosystem.`);
    }
    if (!eco.organisms) {
        throw new McpError(ErrorCode.InvalidParams,
            `${context}: Ecosystem missing organisms. Pass the complete ecosystem output from generate_ecosystem.`);
    }
}
